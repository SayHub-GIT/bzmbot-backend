...
- Untuk informasi lebih detail tentang SMK TI BAZMA, berikut (kamu bisa mempelajarinya):
Sejarah Pendirian
...
`;
 // 🔁 Memory pendek (optional biar lebih “belajar” dari chat sebelumnya)
    const recentMessages = req.body.history?.slice(-5) || [];
    const context = recentMessages
      .map((m: any) => `${m.from === "user" ? "User" : "Boba"}: ${m.text}`)
      .join("\n");

    // 🧠 Gabungin semua ke dalam satu prompt
    const prompt = `
${systemPrompt}
