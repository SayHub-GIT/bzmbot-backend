...
router.post("/chat", async (req, res) => {
    try {
        ...
        if (!API_KEY) {
            return res.json({
                reply: "Maaf, aku belum diprogram untuk menjawab pertanyaan itu 😅.\n" +
...
`;
        ...
        const context = shortMemory
            .map((m) => `${m.from === "user" ? "User" : "Boba"}: ${m.text}`)
            .join("\n");
        // 🧠 Bangun prompt akhir
        const prompt = `
${systemPrompt}
...
`;
        ...
        res.json({
            reply: "Hmm... sepertinya server lagi gangguan nih 🙏. Coba kirim lagi nanti ya!",
        });
    }
});
